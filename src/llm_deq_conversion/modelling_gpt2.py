    ...
    def _run_blocks(
        ...
        **kwargs,
        ...
        hidden_states = self.adapter(
          torch.cat([input_embeds, hidden_states.reshape_as(input_embeds)], dim=-1)
        )
        hidden_states = hidden_states + position_embeds.to(hidden_states.device)
        # Attention mask.
        ...
        if self.config.add_cross_attention and encoder_hidden_states is not None:
            ...
            if _use_sdpa:
                encoder_attention_mask = _prepare_4d_attention_mask_for_sdpa(
                    mask=encoder_attention_mask, dtype=hidden_states.dtype, tgt_len=input_shape[-1]
                )
        ...
        if deq_steps > 0:
            with torch.no_grad():
                hidden_states, _, stats = self.solver(
                    ...
                    tau=1.0,
                    stop_mode='rel',
                    return_final=self.return_final and self.training, 
                )
...
class EBGPT2Model(GPT2Model):
    def __init__(self, config: GPT2Config, num_grad_steps: int, step_size: float):
        super().__init__(config)
        self.num_grad_steps = num_grad_steps
        self.step_size = nn.Parameter(torch.tensor(float(step_size)), requires_grad=True)
        self.energy_layer = nn.Linear(config.hidden_size, 1)
        self.adapter = nn.Linear(2*config.hidden_size, config.hidden_size)
    ...
    def _run_blocks(
        ...
        **kwargs,
        ...
        # Final layer normalization
        hidden_states = self.ln_f(hidden_states)
        energy = self.energy_layer(hidden_states)
        return energy
    ...
    def forward(
        ...
        **kwargs,
        ...
        position_embeds = self.wpe(position_ids)
        output_shape = (-1,) + input_shape[1:] + (inputs_embeds.size(-1),)
        hidden_states = torch.randn_like(inputs_embeds)

        energy_forward = lambda x: self._run_blocks(
            input_embeds=inputs_embeds,
            hidden_states=x,
            position_embeds=position_embeds,
            past_key_values=past_key_values,
        ...
        with torch.set_grad_enabled(True):
            for step in range(self.num_grad_steps):
                hidden_states = hidden_states.detach().requires_grad_().reshape(inputs_embeds.shape)
                # simplifying alot the code
                energy_preds = energy_forward(hidden_states)
                print(energy_preds.shape)
                energy_preds = energy_preds.reshape(-1, 1)
                print(energy_preds.sum())
                hidden_states_grad = torch.autograd.grad([energy_preds.sum()], [hidden_states], create_graph=self.training and step == (self.num_grad_steps - 1))[0]
                if torch.isnan(hidden_states_grad).any() or torch.isinf(hidden_states_grad).any():
                    raise ValueError("NaN or Inf gradients detected during MCMC.")
                hidden_states = hidden_states - self.step_size * hidden_states_grad
...
class EBGPT2LMHeadModel(GPT2LMHeadModel):
    def __init__(self, config: GPT2Config, num_grad_steps: int, step_size: float):
        super().__init__(config)
        self.transformer = EBGPT2Model(config, num_grad_steps=num_grad_steps, step_size=step_size)
        self.post_init()
...
if __name__ == "__main__":
    ...
    model = EBGPT2LMHeadModel(config, num_grad_steps=5, step_size=500.0)
